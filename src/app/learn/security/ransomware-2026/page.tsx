...
要点：ランサムウェアは攻撃の“最後”。手前の各段で止めるほど被害は小さい`}</code>
      ...
      <h2>実体験：バックアップの考え方が変わった話</h2>
      <p>
        自分自身がランサムウェアの被害に遭った経験はありませんが、バックアップについては個人開発をする中でかなり意識するようになりました。以前は、GitHub にコードがあるので「基本的には大丈夫だろう」と考えていた部分もありました。
      </p>
      <p>
        ただ、ランサムウェアについて調べると、単純に PC 上のファイルを暗号化するだけでなく、バックアップまで狙われるケースがあることを知りました。そこで、バックアップは「同じ PC の別フォルダにコピーしておけばいい」というものではなく、<strong>元データと同時に被害を受けない場所・方法でバックアップを持つことが重要</strong>なんだと考えるようになりました。個人開発でも、GitHub などのリモートリポジトリにコードを置いておくことや、重要なデータを一か所だけに保存しないことを意識しています。ランサムウェアというと企業向けの大きなインシデントというイメージがありましたが、個人でも「もし PC が突然使えなくなったら、このデータは復元できるのか」と考えるきっかけになりました。
      </p>
