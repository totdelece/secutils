...
→ {
    ...
    ...
      ...
      <h2>実体験：secutils に「ログイン機能」がない理由</h2>
      <p>
        本サイトには、あえてログイン機能を付けていません。一番大きい理由は、そもそもユーザーの入力データをサーバーに保存しないという設計にしているからです。パスワード生成やハッシュ生成のようなツールでは、ユーザーが入力したデータをサーバーに送信・保存する必要はなく、ブラウザ上で完結させた方がシンプルです。ログイン機能を追加すると、ユーザー情報を管理する DB、セッション、パスワードリセット、認証情報の保護など、セキュリティ上考えるべきことが一気に増えます。「ログイン機能があるサイトの方がちゃんとして見える」という考え方もあると思いますが、<strong>機能上必要ないものをわざわざ持たない方がセキュリティ的にも良い</strong>と考え、認証機能自体を持たせない設計にしました。
      </p>
      <p>
        一方で、個人開発の別プロジェクトで OAuth 連携を組み込もうとしたときに、最初に戸惑ったのは <code>redirect_uri</code> でした。「Google でログインする」というだけなら簡単そうに見えますが、実際には Google 側に登録した redirect_uri と自分のアプリ側の URI が完全一致していないとエラーになります。開発環境では localhost、本番では本番ドメインになるので、それぞれ設定を分ける必要があり、「ログインボタンを作るだけ」では終わらないと実感しました。scope についても、最初は「とりあえず必要そうなものを全部指定すればいい」と考えがちでしたが、ユーザーの情報にアクセスする権限を要求するものなので、<strong>必要最低限にするべき</strong>だと学びました。OAuth は便利な仕組みですが、裏側ではリダイレクト先や権限、トークンなど、かなりセキュリティを意識しないといけない仕組みだと実感しています。
      </p>
      <p>
        当時は理由が分からず「なぜ完全一致でないといけないのか」と少し面倒に感じていたのですが、後述する<strong>「redirect_uri の完全一致」</strong>の項目にある通り、ここを緩めると認可コードを別のURLに誘導される攻撃につながることを知り、自分がつまずいたあの厳格さには理由があったのだと納得しました。
      </p>
