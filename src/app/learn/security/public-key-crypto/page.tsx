...
decrypted = RSA_decrypt(encrypted, private_key) // 秘密鍵を持つ人だけ開錠可能`}</code></pre>
  ...
  受信者: verify("hi", signature, Alice_public_key) → True/False`}</code></pre>
      ...
      <h2>実体験：「どっちの鍵が何をするんだっけ」を整理した話</h2>
      <p>
        公開鍵暗号を勉強し始めたときは、正直かなり混乱しました。「公開鍵で暗号化したものは秘密鍵で復号する」という話と、「秘密鍵で署名して公開鍵で検証する」という話が両方出てくるので、「結局どっちが何をする鍵なんだ」となっていました。特に RSA を勉強していたときは、暗号化と電子署名の話が頭の中で混ざっていました。
      </p>
      <p>
        そこで、<strong>「秘密鍵は本人しか持っていないもの」「公開鍵は他人に渡してもいいもの」</strong>とだけ考えて、暗号化と署名を別々に整理したら、かなり理解しやすくなりました。数学的な仕組みから覚えようとすると難しいのですが、「誰に読まれたくないのか」「誰が本人だと証明したいのか」という目的から考えると腑に落ちました。
      </p>
      <p>
        HTTPS や SSH は以前から普通に使っていましたが、公開鍵暗号を勉強してから「これも公開鍵暗号の実例だったのか」と改めて気づきました。特に SSH で公開鍵をサーバーに登録し、秘密鍵を自分の PC 側で管理する仕組みは、公開鍵と秘密鍵の役割をそのまま体験できるので分かりやすかったです。それまでは「SSH 接続のための設定」くらいにしか考えていませんでしたが、公開鍵は相手に渡してもよく、秘密鍵は自分だけが持つという考え方そのものだったのだと、後からつながりました。HTTPS の証明書も同じで、ブラウザの「鍵マーク」だけを見ていた頃と、仕組みを勉強した後では見え方がかなり変わりました。
      </p>
      <p>
        「暗号化」と「署名」を目的から切り分けて整理したこの考え方は、後述する<strong>「HMAC と公開鍵署名の違い」</strong>を理解する土台にもなりました。HMAC は双方が同じ鍵を持つので「誰でも検証できるわけではない」、公開鍵署名は「本人にしか署名できないが、検証は誰でもできる」——この違いも、暗号化と署名を混同していた頃には理解できなかった部分です。
      </p>
