...
export default function Page() {
  ...
  base-uri 'none'`}</code></pre>
      ...
      <h2>実体験：secutils を作りながら気づいたこと</h2>
      <p>
        本サイトには、入力した文字列をそのまま結果として表示するツールが少なくありません。作り始めた当初は「普通の文字列を入力して、普通の文字列として表示する」ことしか考えていませんでしたが、XSS について調べるうちに、<code>&lt;script&gt;</code> のような分かりやすいものだけでなく、<strong>HTML タグや属性として解釈されてしまうケース</strong>まで考える必要があると気付きました。実際に <code>{"<script>alert(...)</script>"}</code> のような文字列を自分で各ツールに入力してみて、意図せず HTML として解釈されないかを確認するようになりました。セキュリティツール自体が攻撃用の文字列を入力される可能性の高いサイトでもあるので、<strong>「入力値は全部信用しない」</strong>という原則を、他のサイトを作るとき以上に意識するようになりました。
      </p>
      <p>
        React を使っているので、JSX で変数を表示する場合は基本的に自動でエスケープされることも途中で理解しましたが、それを知って一度「React だから安全」と少し油断しかけたこともあります。実際には <code>dangerouslySetInnerHTML</code> のような仕組みを使えば話は変わりますし、外部の HTML を扱う部分では別途注意が必要です。<strong>「React がどこまで守ってくれて、どこからは自分で責任を持つ必要があるのか」</strong>を意識するようになったのは、大きな学びでした。
      </p>
      <p>
        この体験は、後述する<strong>「誤解1: 入力をサニタイズすれば安全」</strong>とも直結しています。入力時にどれだけ気をつけて文字列を弾いても、実際に出力される文脈（HTML 本文なのか、属性値なのか、JS 文字列なのか）まで意識しないと防ぎきれない、というのを実際に自分のツールで手を動かして確かめたことで、理屈だけでなく実感として理解できました。
      </p>
