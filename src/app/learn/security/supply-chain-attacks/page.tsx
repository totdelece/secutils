...
★ 守る要点：①取り込む前に止める ②install時の自動実行を抑える
      ...
      <h2>実体験：本サイトでの依存パッケージの選び方</h2>
      <p>
        本サイト自身も Next.js やその他いくつかの npm パッケージの上に成り立っており、他人事ではありません。パッケージを追加するかどうかは、<strong>ダウンロード数だけで判断しない</strong>ようにしています。見ているのは、GitHub リポジトリが公開されているか、直近もメンテナンスされているか、利用者がある程度いるか、Issue が長期間放置されていないか、といった複数の要素の組み合わせです。
      </p>
      <p>
        もう一つ意識しているのが、<strong>「そのパッケージ、本当に必要か」</strong>という問いです。数行で自分で実装できる処理のために、素性のよく分からないパッケージを1つ追加するくらいなら、自分で書いた方が依存関係を増やさずに済みます。セキュリティ系のサイトを作っている以上、<strong>「有名だから安全」とは限らない</strong>という意識は特に強く持つようにしています。人気パッケージであっても、結局は外部のコードを自分のアプリケーションに取り込むことに変わりはありません。
      </p>
      <p>
        この記事の主旨を、身近な例で実感したこともあります。本サイトで <code>npm audit</code> を実行すると、Next.js が内部で使っている <code>postcss</code> のバージョンに起因する脆弱性が指摘されることがあります。これは自分たちが直接 <code>postcss</code> をインストールしているのではなく、<code>node_modules/next</code> の中にぶら下がった<strong>間接依存</strong>です。<code>npm audit fix --force</code> で直そうとすると、まだ動作検証していない Next.js のバージョンへ強制的に上げることになり、かえって別の不具合を持ち込みかねないため、現状は静観しています。<strong>「自分のコードには問題がなくても、使っているライブラリ経由で問題が生じる」</strong>ことを、身をもって実感した場面です。
      </p>
      <p>
        こうした経験もあって、上の6類型のうち特に<strong>①タイポスクワッティングや③slopsquatting</strong>のように、パッケージを選ぶ・名前を確認する段階で防げる種類のものには特に気を使うようになっています。逆に⑥自己増殖ワームのように「取り込んでしまった後」で被害が連鎖するタイプは、個人開発の規模では検知が難しいと感じており、依存を増やしすぎないことが結果的に一番の対策になっていると考えています。
      </p>
