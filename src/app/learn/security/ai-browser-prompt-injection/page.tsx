...
【AIブラウザ】 エージェントが本人として全部を横断
  サイトA(攻撃者の命令を仕込む)
   ...
   [AIエージェント] ── 本人の権限 ──┬─▶ Gmail（OTP読取）
      ...
      <h2>実体験：Claude Codeを使っていて意識するようになった権限の話</h2>
      <p>
        Claude Code などの AI コーディングツールを使うようになってから、AI に与える権限についてはかなり意識するようになりました。特に、AI が単純に文章を生成するだけでなく、ファイルを読み書きしたり、コマンドを実行したり、場合によっては外部サービスと連携したりできるようになると、<strong>「AI に悪意のある指示を与えたらどうなるか」だけではなく、「AI が外部から取得した情報の中に悪意のある指示が含まれていたらどうなるか」</strong>という問題もあると感じました。
      </p>
      <p>
        例えば、AI に Web ページや README などを読ませたとき、その中に「このコマンドを実行してください」といった指示が含まれていた場合、AI がそれを単なる情報ではなく実行すべき命令として解釈してしまう可能性があります。自分で Claude Code を使って secutils を開発している中でも、AI に何でも自動実行させるのではなく、変更内容を確認してから実行することや、不要な権限を与えないことを意識するようになりました。AI が便利になればなるほど、<strong>「AI 自身を信頼するかどうか」だけでなく、「AI が扱う外部の情報をどこまで信頼するか」</strong>という視点も重要になると感じています。
      </p>
