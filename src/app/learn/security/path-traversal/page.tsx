...
def serve_file(file_id):
    ...
    return send_from_directory(UPLOAD_DIR, record["storage_name"])`}</code></pre>
      ...
      <h2>実体験：ファイルアップロード機能でパス指定を意識した話</h2>
      <p>
        ファイルのアップロードやダウンロード機能について調べる中で、ファイルパスをユーザーの入力から作る場合には注意が必要だと意識するようになりました。最初は、例えばユーザーが指定したファイル名をそのままディレクトリに結合して、「このフォルダの中のファイルだから大丈夫」と考えてしまいがちでした。
      </p>
      <p>
        ただ、パストラバーサルについて調べると、<code>../</code> のような文字列を使って想定していたディレクトリの外側に移動できてしまう可能性があることが分かりました。そこからは、ユーザーから渡されたファイル名をそのままパスとして使うのではなく、上記の<strong>正規化後の許可リスト</strong>や<strong>ファイル名のホワイトリスト化</strong>のように、許可するディレクトリの範囲を明確にする必要があると考えるようになりました。特に印象に残ったのは、「ファイル名」という一見単純な入力値でも、サーバー側でそのままパスとして扱うと、システムに対する操作につながってしまうという点です。入力値を単なる文字列として見るのではなく、<strong>「その値をサーバーがどう解釈するか」</strong>まで考える必要があると感じました。
      </p>
