...
方式  cost ソルト(22文字)         ハッシュ(31文字)`}</code></pre>
  ...
  parallelism (p) = 1`}</code></pre>
      ...
      <h2>実体験：Bcrypt Hasher を作って理解が変わったこと</h2>
      <p>
        <Link href="/tools/bcrypt">Bcrypt Hasher</Link> を作るまでは、正直「パスワードをハッシュ化しておけば安全」というくらいの認識でした。調べてみると、bcrypt には<strong>コスト係数</strong>があり、ハッシュを一回計算するのではなく、あえて計算コストを高くすることで総当たり攻撃を遅くしていると知りました。最初は「だったら一番高いコストにすればいいのでは」と思ったのですが、実際には認証のたびにその計算コストがかかるので、<strong>サーバー側の負荷とのバランス</strong>も必要なんだと理解しました（上の表の通りです）。
      </p>
      <p>
        ソルトについても、最初は「パスワードに何か文字列を付け足すもの」くらいの理解でしたが、<strong>パスワードが同じでもハッシュ結果を変えることで、事前計算されたハッシュ（レインボーテーブル）との照合を難しくする</strong>という役割があると知って、イメージが変わりました。学習の初期には、パスワードをそのまま DB に保存するようなサンプルコードや、単純な MD5 ハッシュを使う例を見たこともあります。当時は「動けばいい」という感覚で、それほど深く考えていませんでしたが、「もし DB が漏れたらパスワードがそのまま持っていかれる」と分かってからは、かなり危険だと感じるようになりました。<strong>「ハッシュ」と「パスワード専用ハッシュ」は別物として考える</strong>必要があると意識するようになったのは、大きな変化でした。
      </p>
      <p>
        この記事にある通り、現代の推奨値は cost=12（約0.3秒）ですが、当時の自分は「とにかく高くすればいい」としか考えておらず、<strong>サーバー負荷との兼ね合いという発想がすっぽり抜けていた</strong>ことになります。数字の根拠を知ってから逆算すると、自分が最初に思い浮かべた「一番強いコスト」がいかに雑な発想だったかがよく分かります。
      </p>
