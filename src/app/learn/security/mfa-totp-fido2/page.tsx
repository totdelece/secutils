...
export default function Page() {
  return (
    <ArticleLayout article={article}>
      ...
      <h2>実体験：TOTP Generator を作って理解が変わったこと</h2>
      <p>
        <Link href="/tools/totp">TOTP Generator</Link> を作る前は、「認証アプリに表示される6桁の数字が30秒ごとに変わる」という程度の理解でした。実際に仕組みを調べてみると、単純にランダムな数字を生成しているわけではなく、<strong>サーバーと認証アプリが共有している秘密鍵と現在時刻を使って同じコードを計算している</strong>だけだと分かりました。サーバー側と認証アプリ側で同じ秘密情報を持っている必要があることや、多少の時刻のズレを考慮する必要があることも、ここで初めて理解できました。「30秒ごとに変わる6桁の数字」という見た目だけだったものが、裏側の仕組みまでイメージできるようになったのは大きな収穫でした。
      </p>
      <p>
        普段使っている MFA は Google Authenticator と Passkey です。認証アプリは一度設定してしまえばかなり便利ですが、<strong>スマホを持っていないとログインできない</strong>という面倒さがあります。一方で Passkey は、対応しているサービスなら指紋認証や端末認証だけでログインできるので、コード入力の手間がなく楽だと感じます。ただし<strong>サービスによって対応状況が違う</strong>ことや、端末を変えたときに「この Passkey はどこに保存されているんだっけ」となることもあり、仕組みを理解していないと戸惑う部分もあると実感しています。
      </p>
      <p>
        端末を変えたときのこの戸惑いは、後述する<strong>「復旧手段（Recovery）の話」</strong>とそのままつながっています。Passkey は便利な反面、同期の仕組みを理解していないと「復旧できるのか、できないのか」が分かりにくく、結局 SMS などの弱い経路に頼ってしまうと元も子もありません。自分が実際に戸惑った経験からも、<strong>便利な認証方式ほど復旧経路まで含めて確認しておく</strong>ことの大切さを感じています。
      </p>
