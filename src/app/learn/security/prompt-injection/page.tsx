...
★ ①②③が揃う＝Lethal Trifecta。1本断てば致命傷を回避`}</code>
      ...
      <h2>実体験：Claude Codeで外部データを読ませるときに意識していること</h2>
      <p>
        これは Claude Code などの AI コーディングツールを使う中で、かなり意識するようになりました。AI にコードを書いてもらうだけなら、基本的には自分が入力したプロンプトが指示になります。ただ、AI がファイルを読んだり、Web ページの内容を取得したりするようになると、その外部データの中に「この指示を実行してください」といった悪意のある文章が含まれている可能性があります。
      </p>
      <p>
        そのときに、それが単なるデータなのか、それとも AI が従うべき命令なのかを AI 自身が完全に区別できるとは限らないというのが怖いところだと感じました。例えば、GitHub の README や外部サイトの文章を AI に読ませているときに、その中に悪意のある指示が混ざっていて、AI がそれを命令として解釈してしまう可能性があります。そのため、Claude Code を使うときも、<strong>AI にファイルや外部サービスへのアクセス権限を必要以上に与えないこと</strong>や、<strong>AI が実行しようとしているコマンドやファイル変更を確認してから許可すること</strong>を意識しています。上記の Lethal Trifecta の考え方で言えば、①機微データへのアクセスと③外部への送信のどちらか、あるいは両方を絞ることを日常的に意識している形です。
      </p>
      <p>
        AI は非常に便利なので、つい「自分の指示どおりに動くもの」と考えてしまいますが、実際には AI が読み込む外部データも攻撃対象になり得るということを知ってから、<strong>AI ツールの権限や承認プロセスまで含めてセキュリティを考える</strong>ようになりました。
      </p>
