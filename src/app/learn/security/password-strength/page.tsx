...
試行回数 ≈ 2^37.6 ≈ 2,000億通り`}</code></pre>
      ...
      <h2>実体験：Password Generator を作って変わったパスワード観</h2>
      <p>
        本サイトの <a href="/tools/password-generator">Password Generator</a> を作る前は、パスワードは「大文字・小文字・数字・記号を混ぜて、ある程度複雑にしておけばいい」という認識でした。ただ、実際にエントロピーや総当たり攻撃について調べていくと、複雑さだけでなく、<strong>パスワードの長さや使い回しの方が重要</strong>だと分かりました。
      </p>
      <p>
        それからは、サービスごとに別のパスワードを使うことをかなり意識するようになりました。全部を自分で覚えるのは現実的ではないので、上のガイドラインにもある通り<strong>パスワードマネージャー</strong>を使って、サービスごとに長くてランダムなパスワードを生成するようになりました。「自分が覚えられるパスワードを作る」という考え方から、「<strong>自分が覚える必要のないパスワードを生成して管理する</strong>」という考え方に変わったのは大きな変化でした。Password Generator を実際に作ったことで、パスワードも「人間が覚えやすいもの」ではなく、「コンピューターに安全に生成・管理させるもの」という考え方が強くなりました。
      </p>
