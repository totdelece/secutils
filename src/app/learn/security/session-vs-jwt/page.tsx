...
Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWI...`}</code></pre>
      ...
      <h2>実体験：個人開発で認証方式を選んだときのこと</h2>
      <p>
        個人開発で認証を考えたときに、セッション方式と JWT 方式のどちらにするか実際に迷ったことがあります。最初は JWT の方が「モダンで便利そう」というイメージがあり、サーバー側でセッションを管理しなくていい分、なんとなくシンプルに見えていました。ただ実際に考えてみると、ログアウトしたときにどうするか、トークンが漏れた場合にどうするか、有効期限をどうするかなど、<strong>JWT だから全部解決するわけではない</strong>ことが分かりました。最終的には、必要以上に複雑な認証方式を採用するより、サービスの規模や構成に合った方法を選ぶべきだと考えるようになり、「JWT の方が新しいから良い」という選び方はしなくなりました。
      </p>
      <p>
        本サイトの <Link href="/tools/jwt-decoder">JWT Decoder</Link> を作っていて感じたのは、JWT はデータを持ち運ぶのには便利な一方、<strong>その分サイズが大きくなりやすい</strong>ということです。ヘッダー・ペイロード・署名を1つのトークンにまとめられるのは便利ですが、Cookie や Authorization ヘッダーで毎回送ることを考えると、セッション ID のような短い値に比べて通信量が増える場合があります。また、一度発行した JWT をサーバー側から簡単に無効化できるわけではないので、「ステートレスだから便利」というメリットの裏には制約があるのだと感じました。Decoder を作る前は、JWT を単純に「安全な認証トークン」くらいに考えていましたが、実際には<strong>どう発行し、どこに保存し、どう失効させるかまで含めて考えるもの</strong>だと理解が変わりました。
      </p>
      <p>
        「即時失効できない」という気づきは、後述する<strong>「リフレッシュトークン: 寿命の使い分け」</strong>の発想にもつながっています。トークンを1種類だけで運用しようとしていた頃は気付きませんでしたが、短命なアクセストークンと長命なリフレッシュトークンを分けるという設計は、まさにこの制約を回避するための工夫だったのだと後から理解しました。
      </p>
