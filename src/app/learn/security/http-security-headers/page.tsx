...
export default function Page() {
  ...
  upgrade-insecure-requests`}</code></pre>
      ...
      <p>
        この一件があってから、CSP を変更するときは後述する<strong>「段階的導入のコツ」</strong>で紹介している <code>Content-Security-Policy-Report-Only</code> を先に試し、コンソールや違反レポートで「何が実際に読み込まれているか」を確認してから本番の CSP に反映する、という手順を踏むようになりました。いきなり本番に厳しい設定を入れて壊す、という同じ失敗を繰り返さないための習慣です。
      </p>
