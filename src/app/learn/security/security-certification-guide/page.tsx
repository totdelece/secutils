...
export default function Page() {
  return (
    <ArticleLayout article={article}>
      ...
      <p>
        筆者自身は基本情報技術者試験、応用情報技術者試験、情報処理安全確保支援士試験を取得している。基本情報では IT の基礎を広く勉強し、応用情報ではネットワークやデータベース、セキュリティなどをもう一段深く勉強した。その上でセキュリティをさらに掘り下げたいと思い、情報処理安全確保支援士まで勉強した。
      </p>
      <p>
        勉強方法は、最初に参考書で全体を一通り理解し、その後に過去問を繰り返し解いて知識を定着させるという方法が中心だった。特に応用情報やセキスペは単純な用語暗記だけでは対応できない問題も多く、「この攻撃はなぜ成立するのか」「この対策をすると何が変わるのか」という仕組みまで理解することを意識していた。
      </p>
      <p>
        資格の勉強を通じて実際に活きていると感じるのは、ネットワークとセキュリティの基礎知識だ。本サイトでは CORS や CSRF、HTTP セキュリティヘッダー、公開鍵暗号などを扱っているが、これらは個別の知識として覚えたというより、基本情報・応用情報で学んだ HTTP・ネットワーク・暗号・認証の知識が土台になっている。セキスペの勉強では「この攻撃にはこの対策」という暗記ではなく、攻撃がどう成立し、どこを守れば防げるのかを考える問題が多かったため、本サイトの記事を書く上でも役立っている。資格の勉強で得た知識をそのままサイトに載せているというより、<strong>資格勉強で身につけた基礎知識を、実際にツールを作ったり記事を書いたりする中で使い直している</strong>、という感覚に近い。関連する国内資格の詳細は<a href="/learn/security/sc-exam-guide">情報処理安全確保支援士 勉強方法・合格率</a>も参照してほしい。
      </p>
