...
要点：検査の“盲点（画像）”と防御の“境界外（個人スマホ）”を同時に突く`}</code>
      ...
      <h2>実体験：QRコードを読み取る前に確認するようになったこと</h2>
      <p>
        QRコードについては、最近は特に「QRコードだから安全とは限らない」と意識するようになりました。例えば、お店や施設にあるQRコードを読み取るときも、リンク先のドメインを一度確認するようにしています。QRコードはスマートフォンのカメラで読み取ると、そのままブラウザが開いてしまうことがあるので、リンク先を意識しないままアクセスしてしまいやすいところが怖いと思っています。
      </p>
      <p>
        特に、駐車場の精算や決済など、お金に関係するQRコードについては、貼り替えられていないか、リンク先が本来のサービスのドメインになっているかを確認した方がいいと感じます。以前はQRコードを見ると「URLを入力する手間が省ける便利なもの」という感覚でしたが、フィッシングについて調べてからは、<strong>「QRコードも結局はURLを隠して表示しているだけなので、リンク先を確認する必要がある」</strong>と考えるようになりました。
      </p>
