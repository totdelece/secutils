...
<script>document.forms[0].submit()</script>`}</code></pre>
      ...
      <h2>実体験：SameSite は「設定すれば終わり」ではなかった</h2>
      <p>
        個人開発で認証まわりを実装していたとき、「この処理って別サイトから勝手にリクエストを送られたらどうなるんだろう」とふと考えたのが CSRF を調べ始めたきっかけでした。最初に「攻撃者が偽のリクエストを送りつける」という説明だけを読んだときはピンとこなかった記憶がありますが、腑に落ちたのは、<strong>Cookie ベースの認証</strong>と組み合わせて考えてからでした。ユーザー本人が何も操作していなくても<strong>ログイン中のブラウザが自動で Cookie を付けて送ってしまう</strong>という一点さえ理解すれば、あとの防御策は自然に頭に入ってきます。
      </p>
      <p>
        個人開発で認証まわりを実装したときは、最初は「<code>SameSite</code> さえ設定すれば CSRF 対策は完了」くらいの認識でした。しかし実際に設定してみると、上の表の通り <code>Strict</code> / <code>Lax</code> / <code>None</code> で挙動がまったく違い、<strong>単純に一番厳しい <code>Strict</code> にすればいいわけではない</strong>ことに気付かされました。<code>Strict</code> は外部サイトのリンクから遷移してきた直後も Cookie が送られないため、「他サイトのリンク経由でもログイン状態のまま来てほしい」ようなフローでは、ユーザーから見ると意図せずログアウトしているように見えてしまうことがあります。セキュリティは強ければ強いほど良いわけではなく、<strong>サービスの仕様やユーザー体験とのバランス</strong>で選ぶものだと実感した部分です。
      </p>
      <p>
        この経験から、認証まわりを作るときは「とりあえず <code>SameSite</code> を設定して終わり」にせず、<strong>そのサービスが外部サイトからの遷移をどこまで許容する必要があるか</strong>まで考えてから値を選ぶようになりました。
      </p>
