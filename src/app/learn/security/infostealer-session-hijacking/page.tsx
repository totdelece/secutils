...
要点：MFAは「入口の瞬間」だけを守る。発行後のCookieは別途守る必要がある。`}</code>
      ...
      <h2>実体験：「新しいデバイスからログイン」通知で焦った話</h2>
      <p>
        Google や SNS などを使っていて、「新しいデバイスからログインがありました」という通知が来たことはあります。実際には自分が別の端末からログインしただけだったのですが、最初に通知を見たときは「誰かにログインされたのでは」と少し焦りました。そのときにログイン履歴や接続中のデバイスを確認して、必要であれば全デバイスからログアウトできる機能があることも確認しました。
      </p>
      <p>
        インフォスティーラーについて調べてからは、パスワードが漏れなければ大丈夫というわけではなく、ブラウザに保存されているセッション Cookie などが盗まれると、攻撃者がパスワードを入力せずにログイン状態を悪用できる可能性があることも意識するようになりました。そのため、怪しいログイン通知が来た場合には、単純にパスワードを変更するだけでなく、<strong>ログイン中のデバイスやセッションを確認して、必要なら全セッションを無効化する</strong>という対応も重要なんだと考えるようになりました。
      </p>
