...
if parsed.scheme not in {"http", "https"}:
    raise ValidationError()`}</code></pre>
      ...
      <h2>実体験：外部URLを取得する機能を考えたときの気づき</h2>
      <p>
        外部URLを取得するような機能を考えたときに、「ユーザーが入力したURLをサーバーからそのまま取りに行かせるのは危ないのではないか」と意識するようになりました。最初は、URLを受け取ってHTTPリクエストを送るだけなら、それほど難しい機能ではないと思っていました。
      </p>
      <p>
        ただ、SSRFについて調べると、ユーザーが指定したURLが外部のWebサイトとは限らず、<strong>サーバー自身や内部ネットワークのアドレスを指定できてしまう可能性</strong>があることが分かりました。上記の<strong>Webhook送信機能</strong>のように、一般ユーザーからはアクセスできない内部の管理画面やクラウドのメタデータサービスに、サーバー自身からアクセスできてしまうケースがあるということです。そこから、外部URLを扱う機能では「URLとして正しいか」だけでなく、<strong>「そのURLにサーバーからアクセスして本当に大丈夫なのか」</strong>という視点が必要なんだと理解しました。SSRFを調べるまでは、URL入力欄は単純な入力値の一つだと思っていましたが、サーバー側からリクエストを送る機能では入力値の持つ意味がまったく違うんだと感じました。
      </p>
