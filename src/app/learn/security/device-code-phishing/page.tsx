...
要点：偽サイト不要。本物の認証画面のまま、トークンの“受け取り先”だけが攻撃者`}</code>
      ...
      <h2>実体験：デバイスコードフローを実際に使ったことがあります</h2>
      <p>
      ...
      <p>
        ただ、デバイスコードフィッシングについて調べてからは、この仕組みを悪用して、攻撃者が用意したコードをユーザーに入力させることで、攻撃者側のデバイスをユーザーのアカウントに認証させる手法があることを知りました。そのため、最近は「コードを入力してください」と表示されたときも、単純に指示に従うのではなく、<strong>どのサービスの認証なのか、どこから要求されているコードなのか</strong>を確認するようになりました。便利な認証方式でも、ユーザーが「表示されたコードを入力すればいい」とだけ理解してしまうと、フィッシングに利用される可能性があるというのが印象に残っています。
      </p>
