...
session.execute(text("SELECT * FROM users WHERE id = :id"), {"id": id})`}</code></pre>
      ...
      <h2>実体験：文字列連結でクエリを組んでいた頃</h2>
      <p>
        データベースを触り始めた頃は、SQL を文字列として組み立てる方が分かりやすく感じて、<strong>文字列連結でクエリを作っていた</strong>ことがあります。そのときは「ユーザーが入力した値を検索条件に入れるだけだから大丈夫だろう」くらいの認識でした。SQL インジェクションについて勉強してから、自分がやっていた方法だと入力値によって SQL そのものの構造を変えられてしまう可能性があると知り、「これはかなり危ない書き方だったな」と思いました。
      </p>
      <p>
        プレースホルダやパラメータ化クエリは、最初は少し面倒に感じました。ただ、実際には入力値と SQL の構造を分離するために必要なものだと理解してからは、むしろプレースホルダを使う方が自然になりました。それ以来、SQL を書くときは<strong>「入力値をどう扱うか」を必ず意識する</strong>ようになりました。上で紹介した ORM の <code>queryRawUnsafe</code> のような「危険な抜け道」の存在も、この経験があったからこそ気をつけないといけない部分だと実感を持って理解できました。
      </p>
