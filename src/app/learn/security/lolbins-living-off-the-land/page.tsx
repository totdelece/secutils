...
★ 各ステップは“正常”。点ではなく「線（系列）」で見ないと捕まらない`}</code>
      ...
      <h2>実体験：PowerShellの警告を見て意識が変わった話</h2>
      <p>
        PowerShell やコマンドプロンプトなど、Windows に最初から入っているツールを使っていて、セキュリティソフトや Windows Defender の警告が出ることはありました。ただ、そこで面白いと思ったのが、PowerShell 自体は Windows の正規の機能なのに、攻撃者もスクリプトの実行やファイルの取得などに利用できるという点です。
      </p>
      <p>
        最初は「ウイルス対策ソフトが怪しいプログラムを検出している」というイメージだったのですが、調べてみると、攻撃者がわざわざ専用のマルウェアを持ち込まなくても、OS に最初から存在する正規ツールを悪用できるということが分かりました。それ以来、セキュリティについて考えるときに<strong>「このプログラムは正規のものだから安全」とは単純に考えないようになりました</strong>。例えば PowerShell やコマンドプロンプトも、何を実行したのか、どこからそのコマンドが実行されたのかなど、使われ方まで含めて判断する必要があるということを意識するようになりました。
      </p>
