...
export default function Page() {
  return (
   ...
   ↓ IP 取得完了、通信開始`}</code></pre>
      ...
      <h2>実体験：自宅の機器が増えてDHCPを意識した話</h2>
      <p>
        自宅で PC やスマートフォン、その他のネットワーク機器を増やしたときに、DHCP の仕組みを意識するようになりました。普段は Wi-Fi に接続するだけで自動的に IP アドレスが割り当てられるので、以前は「なぜ勝手に IP アドレスが設定されるんだろう」くらいにしか考えていませんでした。
      </p>
      <p>
        ネットワークについて勉強してからは、ルーターなどの DHCP サーバーが端末に IP アドレスやサブネットマスク、デフォルトゲートウェイ、DNS サーバーなどの情報を自動的に配布していることが分かりました。特に、自分で固定 IP を設定するときに、DHCP で自動取得する場合との違いを意識するようになって、「普段何気なく Wi-Fi につないでいる裏側でも、いろいろなネットワーク設定が自動的に行われているんだな」と感じました。
      </p>
