...
8. フルリゾルバ → PC「93.184.215.14 だよ」`}</code></pre>
      ...
      <h2>実体験：独自ドメインの DNS 設定でつまずいた話</h2>
      <p>
        本サイトの独自ドメイン secutils.jp を設定するとき、DNS の設定は実際に自分で触りました。最初は「ドメインを取得して、Vercel に設定すれば終わり」というくらいに考えていたのですが、実際には A レコードや CNAME を設定して、ドメインと実サービスを結びつける必要がありました。
      </p>
      <p>
        特に印象に残っているのは、設定を変更した直後にすぐ反映されなかったことです。設定自体は間違っていないはずなのに、自分の環境ではまだ以前の状態が表示されることがあり、「設定を間違えたのかな」と何度か確認しました。ここで、上で説明した<strong>TTL</strong>の存在と、DNS キャッシュが残っていると変更がすぐに全体へ反映されるわけではないことを実感しました。それまでは「DNS 設定を変更したらすぐ切り替わる」という感覚だったので、実際に独自ドメインを運用したことで、DNS は単純な名前と IP アドレスの対応表ではなく、<strong>キャッシュも含めて考える必要がある</strong>のだと理解できました。
      </p>
