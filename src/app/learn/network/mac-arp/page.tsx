...
攻撃: PC ─→ 攻撃者(MITM) ─→ ゲートウェイ ─→ Internet
      ...
      <h2>実体験：ルーターの接続機器一覧でMACアドレスを見た話</h2>
      <p>
        自宅のルーターの管理画面で、接続されている機器の一覧を見ることはあります。そこでは PC やスマートフォンなどの IP アドレスだけでなく、MAC アドレスも表示されるので、「この端末はこの MAC アドレスなんだな」と確認したことがあります。
      </p>
      <p>
        ネットワークについて勉強する前は、MAC アドレスと IP アドレスの違いも少し曖昧だったのですが、ARP について勉強してからは、同じ LAN 内で IP アドレスから MAC アドレスを特定して通信しているという関係が理解できるようになりました。特に、自分の PC の IP アドレスが変わっても、そのネットワークインターフェース自体の MAC アドレスは基本的に別物として存在しているということを理解してから、<strong>IP アドレスと MAC アドレスを混同しなくなりました</strong>。ルーターの接続機器一覧を見ると、普段何気なく使っている家庭内ネットワークでも、こうした仕組みが実際に動いているんだと実感できます。
      </p>
