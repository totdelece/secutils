...
省略後:  fe80::1`}</code></pre>
      ...
      <h2>実体験：IP Address Converter を作って気づいたこと</h2>
      <p>
        本サイトの <a href="/tools/ip-converter">IP Address Converter</a> を作るときに IPv4 だけでなく IPv6 も扱う必要があり、改めて IPv6 について調べ直しました。それまでは「IPv4 のアドレスが足りなくなったから桁数を増やしたもの」というくらいの理解でしたが、実際に扱ってみると、単純に数字が長くなっただけでなく、上記の16進数表記や <code>::</code> による省略など、<strong>アドレスの表現方法そのものが IPv4 とかなり違う</strong>ことが分かりました。特に、同じ IPv6 アドレスでも省略の仕方によって見た目が変わるため、Converter では「文字列としてそのまま比較する」のではなく、<strong>内部的に正規化してから扱う</strong>必要があると意識しました。
      </p>
      <p>
        また、自分が普段使っているネットワークでも IPv6 が使われている可能性があることを意識するようになりました。以前は IP アドレスというと <code>192.168.x.x</code> のような IPv4 しかイメージしていませんでしたが、ネットワーク設定を確認すると IPv6 アドレスが実際に存在しており、「普段意識していないだけで IPv6 は身近に使われているんだな」と感じました。
      </p>
