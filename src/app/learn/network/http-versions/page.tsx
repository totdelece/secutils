...
HTTP/3:    HTTP / QUIC (TLS 1.3 込) / UDP / IP`}</code></pre>
      ...
      <h2>実体験：開発者ツールでHTTPバージョンを意識するようになった話</h2>
      <p>
        secutils を公開してから、ブラウザの開発者ツールなどで実際の通信を見る機会が増えて、HTTP のバージョンも意識するようになりました。最初は HTTP/1.1 と HTTP/2 の違いについても、「HTTP/2 の方が新しくて速い」という程度の理解でした。ただ、HTTP/2 では1つの TCP 接続上で複数のリクエストを効率よく処理できることや、HTTP/3 では QUIC を利用して UDP ベースで通信することなどを知ると、それぞれが単純なバージョンアップではなく、<strong>通信効率や遅延への課題を解決するために進化してきたもの</strong>なんだと分かりました。
      </p>
      <p>
        Vercel のような CDN やクラウド基盤を利用すると、自分で Web サーバーを細かく設定しなくても新しいプロトコルの恩恵を受けられるので、「自分では意識していなくても、裏側では HTTP/2 や HTTP/3 が使われているんだな」と感じることがあります。
      </p>
