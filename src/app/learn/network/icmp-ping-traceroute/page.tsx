...
round-trip min/avg/max/stddev = 11.987/12.115/12.345/0.158 ms`}</code></pre>
 ...
 7  edge.example.com (93.184.215.14)   122.789 ms`}</code></pre>
      ...
      <h2>実体験：トラブルシューティングで ping と traceroute を使うようになった話</h2>
      <p>
        ping は、ネットワークの疎通確認をするときに実際によく使っています。例えば、インターネットにつながらないときに、いきなり「Web サイトが落ちている」と考えるのではなく、まず自分の端末からルーターまで通信できているのか、その先まで通信できているのかを確認するために ping を使うことがあります。traceroute についても、通信先までの経路を確認できるので、「どこまでは通信できているのか」を切り分けるときに便利だと感じました。
      </p>
      <p>
        以前は ping を単純に「相手が生きているか確認するコマンド」と考えていましたが、実際にトラブルシューティングで使うようになると、<strong>ネットワークのどの部分に問題があるのかを切り分けるためのツール</strong>という認識に変わりました。
      </p>
