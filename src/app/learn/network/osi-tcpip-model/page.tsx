...
L1  電気信号として送信`}</code></pre>
      ...
      <h2>実体験：トラブルシューティングでレイヤーを意識するようになった話</h2>
      <p>
        基本情報や応用情報の勉強で OSI 参照モデルはかなり勉強しましたが、実際にネットワークのトラブルを調べるときにも、ある程度レイヤーを意識して切り分けるようになりました。例えば、自宅の PC からインターネットにつながらない場合、いきなり Web サイト側の問題だと考えるのではなく、まず Wi-Fi やケーブルなどの物理的な部分、次に IP アドレスやルーティングなどの L3、そこから TCP や DNS、最終的に HTTP などを確認するという考え方です。
      </p>
      <p>
        secutils の運用でも、サイトが表示されない場合に「DNS が正しく引けているのか」「HTTPS の接続自体が成立しているのか」「HTTP のレスポンスに問題があるのか」と順番に切り分けるようになりました。以前は OSI 参照モデルを試験のために暗記している感覚もありましたが、実際のトラブルシューティングでは<strong>「今見ている問題はどの層の話なのか」を考えるためのフレームワーク</strong>として使えることが分かりました。
      </p>
