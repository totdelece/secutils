...
/8  = 11111111.00000000.00000000.00000000 = 255.0.0.0`}</code></pre>
      ...
      <h2>実体験：CIDR / Subnet Calculator を作って理解が深まった話</h2>
      <p>
        CIDR / Subnet Calculator を作る前は、<code>192.168.1.0/24</code> の「/24」が何を表しているのか、正直そこまで具体的にはイメージできていませんでした。ネットワークの範囲を表すものだということは知っていましたが、実際に Calculator を作るために計算してみると、<code>/24</code> なら先頭24ビットがネットワーク部で、残り8ビットがホスト部になるという意味だと理解できました。さらに、<code>/24</code> なら上の表の通り2の8乗で256個分のアドレス空間になる、といったところまで実際に計算するようになってから、単なる「ネットワーク設定に書いてある数字」ではなく、<strong>IPアドレスの範囲を具体的に表しているもの</strong>として見られるようになりました。
      </p>
      <p>
        その後、自宅のネットワーク設定やクラウドの設定画面などで CIDR 表記を見ると、「これが勉強した CIDR か」と気づくようになりました。以前は <code>/24</code> を見ても何となく見ていたのが、今では「このネットワークはこのくらいの範囲なんだな」とすぐイメージできるようになったので、Calculator を作ったことでかなり理解が深まったと感じています。
      </p>
