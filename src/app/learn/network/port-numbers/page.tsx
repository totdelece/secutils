...
└─ :5432 → PostgreSQL`}</code></pre>
      ...
      <h2>実体験：Port Number Reference を作って理解が具体的になった話</h2>
      <p>
        Port Number Reference を作る前は、80番が HTTP、443番が HTTPS、22番が SSH というように、主要なポート番号を個別に暗記しているような状態でした。実際にツールを作ってみると、ポート番号は単なる「サービスごとの番号」ではなく、<strong>IP アドレスと組み合わせて「どの通信先・サービスに接続するのか」を識別するためのもの</strong>だと、より具体的に理解できました。特に、同じサーバーでも80番と443番では別のサービスとして通信できるということを改めて考えると、ポート番号の役割が分かりやすかったです。
      </p>
      <p>
        また、セキュリティの観点では後述の「使わないポートを閉じる」という考え方にもつながるので、単なるネットワークの知識ではなく、セキュリティ設定にも直接関係するものだと感じるようになりました。
      </p>
