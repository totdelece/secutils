...
5. 共有秘密からセッション鍵を導出 → 以降この鍵で対称暗号化通信`}</code></pre>
  ...
  └─ 中間 CA "DigiCert TLS RSA SHA256 2020 CA1" の署名
    └─ ルート CA "DigiCert Global Root CA" の署名
      ...
      <h2>実体験：独自ドメインをHTTPS化して意識したこと</h2>
      <p>
        secutils.jp を独自ドメインで公開するときに、HTTPS について改めて意識しました。Vercel を利用しているので証明書自体は自動で発行・更新されるため、自分で証明書を購入したりサーバーにインストールしたりする必要はありませんでした（裏側では上記の Let&apos;s Encrypt が使われています）。ただ、「ブラウザで https://secutils.jp にアクセスすると、なぜ安全な通信として成立するんだろう」と気になって、TLS や証明書の仕組みを改めて調べました。
      </p>
      <p>
        以前は HTTPS というと単純に「HTTP を暗号化したもの」という認識でしたが、実際には証明書によって接続先の正当性を確認したり、TLS のハンドシェイクを通して安全に通信を開始したりと、いくつかの仕組みが組み合わさっていることが分かりました。自動で HTTPS 化される環境は非常に便利ですが、その裏側で TLS や証明書が動いていることを理解してからは、ブラウザの鍵マークを見るときも少し見方が変わりました。
      </p>
