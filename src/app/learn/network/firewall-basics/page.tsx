...
戻りパケットが届く
   ↓ コネクションテーブルにマッチ → 自動的に通す`}</code></pre>
      ...
      <h2>実体験：Vercel を使っていて感じたファイアウォールとの距離感</h2>
      <p>
        Vercel を使って本サイトを公開しているため、一般的なサーバーを自分で構築するときのように、ファイアウォールの細かい設定を自分で管理する場面はそれほど多くありません。ただ、クラウドサービスの設定を見ていると、<strong>「どこまでアクセスを許可するか」という考え方は、セキュリティヘッダーなどの設定とも共通している</strong>と感じました。
      </p>
      <p>
        セキュリティを勉強する中で、ファイアウォールは「悪い通信を全部ブロックするもの」というより、上の<strong>デフォルト拒否</strong>の考え方の通り「必要な通信だけを許可して、それ以外を制限する」のが基本なんだと理解するようになりました。以前は「80番と443番を開ける」といった設定を単なる作業として見ていましたが、実際には「なぜこのポートを公開する必要があるのか」「この通信を本当に許可する必要があるのか」という視点で考えるものなんだと分かりました。Vercel のようなマネージドサービスでは自分で iptables などを設定する機会はありませんが、クラウドを使う場合でも、どこまで外部に公開するのかを意識することは重要だと感じています。
      </p>
